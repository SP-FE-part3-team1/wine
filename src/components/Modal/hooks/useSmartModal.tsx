...
// 페이지 경로 기반 컨텍스트 분석
type PageContext = 'wine-list' | 'wine-detail' | 'my-wines' | 'my-reviews' | 'admin' | 'unknown';
...
export const useSmartModal = (options: SmartModalOptions = {}) => {
  ...
  const { openWineModal, openReviewModal, openFilterModal, closeModal } = useModal();
  ...
  const analyzePageContext = useCallback((): PageContext => {
    // 더 정확한 경로 매칭
    if (pathname.includes('/wines/') && pathname.match(/\/wines\/\d+/)) return 'wine-detail';
    if (pathname.includes('/wines') || pathname === '/') return 'wine-list';
    if (pathname.includes('/my-wines') || pathname.includes('/profile/wines')) return 'my-wines';
    if (pathname.includes('/my-reviews') || pathname.includes('/profile/reviews')) return 'my-reviews';
  ...
  const inferModalIntent = useCallback((action: string): ModalIntent => {
    ...
    if (lowerAction.includes('edit') || lowerAction.includes('modify') || lowerAction.includes('update')) return 'edit';
    if (lowerAction.includes('filter') || lowerAction.includes('search')) return 'filter';
    if (lowerAction.includes('review') || lowerAction.includes('rate')) return 'review';
    return 'view';
  }, []);

  // 🛡️ ID 안전 변환 (타입 안정성)
  const safeConvertId = useCallback((id: string | number | undefined): string | undefined => {
   ...
   */
  const openSmartModal = useCallback(async (action: string, data?: ModalData) => {
    ...
    // selectAndOpenModal intentionally omitted to avoid circular dependency
  ...
  // 🍷 와인 관련 액션 처리 (비동기)
  const handleWineAction = useCallback(async (intent: ModalIntent, pageContext: PageContext, data?: ModalData) => {
    switch (intent) {
      case 'create':
  ...
  // ⭐ 리뷰 관련 액션 처리 (비동기)
  const handleReviewAction = useCallback(async (intent: ModalIntent, pageContext: PageContext, data?: ModalData) => {
    switch (intent) {
      case 'create':
      case 'review':
        const wineId = safeConvertId(data?.wineId);
  ...
  // 🔍 필터 액션 처리
  const handleFilterAction = useCallback((pageContext: PageContext, data?: ModalData) => {
    const currentFilters = data?.filters || FILTER_DEFAULT_VALUES;
    return openFilterModal(currentFilters);
  }, [openFilterModal]);

  // ➕ 생성 액션 처리 (페이지별 자동 추론, 비동기)
  const handleCreateAction = useCallback(async (pageContext: PageContext, data?: ModalData) => {
    switch (pageContext) {
      case 'wine-list':
      case 'my-wines':
      case 'admin':
  ...
  const handleEditAction = useCallback(async (pageContext: PageContext, data?: ModalData) => {
    switch (pageContext) {
      case 'wine-detail':
        const reviewId = safeConvertId(data?.reviewId);
        const wineId = safeConvertId(data?.wineId);
   ...
   */
  const selectAndOpenModal = useCallback(async (
    action: string, 
    intent: ModalIntent, 
    ...
    switch (action) {
      ...
      case 'review-edit':
        const editWineId = safeConvertId(data?.wineId);
        const reviewId = safeConvertId(data?.reviewId);
        if (!editWineId || !reviewId) throw new Error('와인 ID와 리뷰 ID가 필요합니다.');
        return await openReviewModal('edit', editWineId, reviewId);
  ...
  const smartActions = {
    ...
    filter: (filters?: FilterState) => openSmartModal('filter', { filters }),
    
    // 명시적 액션 (타입 안전)
    createWine: () => openSmartModal('wine-create'),
    editWine: (wineId: string | number) => openSmartModal('wine-edit', { wineId }),
    reviewWine: (wineId: string | number) => openSmartModal('review-create', { wineId }),
    editReview: (wineId: string | number, reviewId: string | number) => 
      openSmartModal('review-edit', { wineId, reviewId }),
    
    // 컨텍스트 기반 액션
    wine: (intent: 'create' | 'edit', data?: ModalData) => openSmartModal('wine', { ...data, intent }),
    review: (intent: 'create' | 'edit', data?: ModalData) => openSmartModal('review', { ...data, intent })
  };
...
export const useQuickModal = (customOptions?: Partial<SmartModalOptions>) => {
  ...
  return {
    ...
    filter: smartModal.filter,   // 필터 열기
